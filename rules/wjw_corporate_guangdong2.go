...
func init() {
	WjwCorporateGuangdong2.Register()
}

var WjwCorporateGuangdong2 = &spider.Spider{
	Name:         "全球五金B",
	Description:  "全球五金[广东35-94] http://www.wjw.cn/gongying/quyu/guangdong",
	EnableCookie: false,
	RuleTree: &spider.RuleTree{
		...
		Trunk: map[string]*spider.Rule{
			...
			"供应列表": {
				ParseFunc: func(ctx *spider.Context) {
					...
					dom.Find(".list-hen li").Each(func(i int, li *goquery.Selection) {
						...
						cpA := li.Find(".lhitem-cp a").First()
						if url, ok := cpA.Attr("href"); ok {
							//公司名称
							corpName := gsA.Text()
			...
			"产品页面": {
				ParseFunc: func(ctx *spider.Context) {
					dom := ctx.GetDom()
					url, ok := dom.Find(".com-detail .title a").First().Attr("href")
					if ok {
						ctx.AddQueue(&request.Request{
							Url:  url,
							Rule: "公司主页",
